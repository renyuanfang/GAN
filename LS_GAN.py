...
class LS_GAN(object):
    ...
    def build_model(self):        
        ...
        #distance between real and fake images
        dist = tf.reduce_sum(tf.abs(self.inputs - G), axis = [1, 2, 3]) * self.lamb
        self.d_loss = tf.reduce_mean(tf.nn.relu(D_real - D_fake + dist))
        #gradient penalty, real output regarding to input
        grad = tf.gradients(D_real, [self.inputs])[0]
        slope = tf.sqrt(tf.reduce_sum(tf.square(grad), axis = [1, 2, 3]))
        self.d_loss += tf.reduce_mean(slope) * self.gamma 
