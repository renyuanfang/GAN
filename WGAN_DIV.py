...
class WGAN_DIV(object):
    ...
    def discriminator(self, x, y=None, reuse=False):
        with tf.variable_scope('discriminator', values=[x], reuse=reuse): 
            with slim.arg_scope(self.disc_arg_scope()):
                net = x
    ...
    def build_model(self):        
        ...
        D_fake = self.discriminator(G, self.y, reuse=tf.AUTO_REUSE)
        #output of D for real images
        D_real = self.discriminator(self.inputs, self.y, reuse=tf.AUTO_REUSE)
        ...
        #sampling on real and fake images individually, do the gradient
        real_grad = tf.gradients(D_real, self.inputs)[0]
        fake_grad = tf.gradients(D_fake, G)[0]
        real_grad_norm = tf.reduce_sum(tf.square(real_grad), axis = [1, 2, 3]) ** (self.p / 2)
        fake_grad_norm = tf.reduce_sum(tf.square(fake_grad), axis = [1, 2, 3]) ** (self.p / 2)
        #following multiply 1/2, because we use fake and real samples together
        grad = tf.reduce_mean(real_grad_norm + fake_grad_norm) * self.k / 2 
    ...
    def train(self):        
        ...
        for epoch in tqdm(range(start_epoch, self.epoch)):
            for _ in range(self.d_iters):
                ...
                _, d_loss, sumStr = self.sess.run([self.d_optimizer, self.d_loss, self.d_sum], feed_dict={self.z:batch_z})
